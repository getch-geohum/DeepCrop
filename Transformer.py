...
class TransformerModel(nn.Module):
    def __init__(self, input_dim=13, num_classes=9, d_model=64, n_head=2, n_layer=5,
                 d_inner=128, activation="relu", dropout=0.1):

        super(TransformerModel, self).__init__()
        self.modelname = f"TransformerEncoder_input-dim={input_dim}_num-classes={num_classes}_d-model={d_model}_d-inner={d_inner}_n-layers={n_layer}_n-head={n_head}_dropout={dropout}"
        ...
        self.relu = ReLU()
        self.transformerencoder = TransformerEncoder(encoder_layer, n_layer, encoder_norm)
...
class Trainer:
    def __init__(self,epochs=100,
        ...
        self.n_head=n_head # 10,
        self.n_layer=n_layer #5,
        self.d_model=d_model #3,
        self.d_inner=d_inner #128
        ...
        self.model = TransformerModel(input_dim=input_size,
...
def argumentParser():
    ...
    parser.add_argument('--n_layer', help='Model number of layers', default=3, type=int, required=False)
    parser.add_argument('--d_model', help='Model dimensions', default=64, type=int, required=False)
    parser.add_argument('--d_inner', help='Model inner dimensions', default=128, type=int, required=False)
    parser.add_argument('--n_head', help='Number of heads for transofrmer', default=2, type=int, required=False)
...
#                         text.write(f'accuracy: {OPTIMIZER.accuracy}, lr: {lr}, hdim: {hdim}, layers: {nlayer}, dropout:{drop}\n')
#                         c+=1
#         text.close()
