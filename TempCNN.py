    ...
    def train(self):
        ...
        for k in range(self.epochs):
            ...
            for jj, (x, y) in enumerate(self.train_loader):
                ...
                # loss = F.nll_loss(logits, y.view(-1).long(), weight=self.weight.float())
...
#                     self.ac_monitor+=1
    ...
    def test(self, read_weight=False, phase='optimize',save_feature=True):
        ...
        for jj, (x, y) in enumerate(self.test_loader):
            x = torch.permute(x, (0,2,1)).to(self.device)
            y = y.to(self.device)
            logs, feat_ = self.model(x.float())
            logs = logs.argmax(-1)
            ...
            if save_feature:
                np.save(f'{self.feature_fold}/{jj}_{self.data}.npy', feat_.cpu().detach().numpy()) # needs place where to save
...
def argumentParser():
    ...
    parser.add_argument('--feature_fold', help='Folder to save te deep features', type=str)
